...
def sample_random_position(occupancy_grid):
    """Sample a valid, random, unoccupied position"""
...
def adjust_pose(node, final_position, occupancy_grid):
    ...
    centre, radius, angle = custom_find_circle(node, final_node)
    # chord = final_position - node.position
    # theta = np.arctan2(chord[Y], chord[X])
    # final_pose[2] = 2 * theta - node.yaw
    final_pose[2] = angle
    final_node = Node(final_pose)
    centre, radius = find_circle(node, final_node)
    if is_collision(node, final_node, centre, radius, occupancy_grid):
        return None

    return final_node

def intersect(vec_a, dir_a, vec_b, dir_b):
    A = np.array([[dir_a[X], -dir_b[X]], [dir_a[Y], -dir_b[Y]]])
    B = vec_b - vec_a
    tu = np.linalg.solve(A, B)
    return vec_a + tu[0] * dir_a
...
def is_anticlockwise(node_a, node_b, mid_point):
    return (
        1
        if (node_b.position[X] - node_a.position[X])
        * (mid_point[Y] - node_a.position[Y])
        - (node_b.position[Y] - node_a.position[Y])
        * (mid_point[X] - node_a.position[X])
        > 0
        else -1
    )

    v = centre - perpendicular_bisect  # Vertex pointing towards centre
    d = np.array([1, 0]).dot(v)
    if d > 0:
        return 1
    elif d < 0:
        return -1
    else:
        d = np.array([0, 1]).dot(v)
        if d > 0:
            return 1
        elif d < 0:
            return -1

def custom_find_circle(node_a, node_b):
    # ! We always assume the circle is draw clockwise. Given this, we only ever need to rotate clockwise to get perp
    # Tangent - in the direction that we're going in
    # print("Call started")
    # na_pose = node_a.pose
    # nb_pose = node_b.pose
    tangent_dir = node_a.direction
    tangent_pos = node_a.position

    chord_dir = node_b.position - node_a.position
    bisector_pos = node_a.position + chord_dir / 2.0
    bisector_dir = normalize(perpendicular(chord_dir))

    normal_dir = perpendicular(tangent_dir)
    normal_pos = tangent_pos

    # ! Assumption - the centre is always a clockwise rotation

    centre = intersect(normal_pos, normal_dir, bisector_pos, bisector_dir)

    radius = np.linalg.norm(centre - node_a.position)
    node_b_norm = node_b.position - centre
    tangent_b = perpendicular(node_b_norm) * is_anticlockwise(node_a, node_b, centre)

    angle = np.arctan2(tangent_b[1], tangent_b[0])
    # assert (np.all(node_a.pose == na_pose))
    # assert(np.all(node_b.pose == nb_pose))
    # print("Call Ended")
    return centre, radius, angle


def is_collision(node_a, node_b, centre, radius, occupancy_grid):
    node_a_norm = node_a.position - centre
    node_b_norm = node_b.position - centre
    change_in_angle = angle_difference(
        node_a_norm, node_b_norm
    )
    arc_length = (np.abs(change_in_angle) / (2 * np.pi)) * np.pi * radius ** 2

    sample_distance = 0.005
    num_samples = math.ceil(arc_length / sample_distance)
    for theta in np.linspace(0.0, change_in_angle, num_samples):
        pos = centre + rotate(node_a_norm, theta)
...
def angle_difference(a, b):
    ...
    return np.arctan2(b_copy[1], b_copy[0]) - np.arctan2(a_copy[1], a_copy[0])
