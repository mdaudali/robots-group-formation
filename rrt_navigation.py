...
def get_nearest_point(position, path_list):
    distance_list = path_list - position
    as_norm = np.linalg.norm(distance_list, axis=1)

    return np.argmin(as_norm)


def get_velocity(position, path_points):
    global last_et, sum_et
    ...
    if np.linalg.norm(position - path_points[-1]) < 0.2:
        return v
    # TODO: Well, the path is recomputed on every iteration, so we don't really need to see which point is the next one
    kp = 1.5
    ki = 0.1  # These thresholds work the best
    kd = 1
    point = path_points[
        min(get_nearest_point(position, path_points) + 1, len(path_points) - 1)
    ]

    error = point - position

    # ! Proportionate control
    proportionate_control = kp * error

    # ! Integral control
    sum_et += error
    if np.linalg.norm(error) > 0.3:
        # ! Robot has re-localised itself - we will reset the integral term to allow it to wind back up.
        print("Integral error reset")
        sum_et = 0

    MIN_INTEGRAL = -1
    MAX_INTEGRAL = 1
    sum_et = np.maximum(np.minimum(sum_et, MAX_INTEGRAL), MIN_INTEGRAL)
    integral_control = ki * sum_et

    # ! Differential control
    differential_control = 0
    if last_et is not None:
        differential_control = kd * (error - last_et)

    last_et = error
    v = proportionate_control + integral_control + differential_control
    print(
        "Proportionate control:",
        proportionate_control,
        "Integral control",
        integral_control,
        "Differential control:",
...
def run(args):
    ...
    while not rospy.is_shutdown():
        ...
        goal_reached = np.linalg.norm(slam.pose[:2] - goal.position) < 0.2
        if goal_reached:
            publisher.publish(stop_msg)
            rate_limiter.sleep()
            continue
        ...
        if current_path and time_since < 2.0:
            rate_limiter.sleep()
            continue
